function CreateExtendedPlayer(player, accounts, inventory, job, job2, loadout, name, lastPosition)
	...
	self.setJob = function(job, grade)
		...
		if ESX.DoesJobExist(job, grade) then
			...
			print(('es_extended: commande %s ignoré, job pas trouvé !'):format(self.source))
		end
	end
...
self.setJob2 = function(job2, grade)
	...
	if ESX.DoesJobExist(job2, grade) then
		...
		self.job2.grade2       = tonumber(grade2)
		...
		TriggerEvent('esx:setJob', self.source, self.job2, lastJob)
		TriggerClientEvent('esx:setJob', self.source, self.job2)
	else
		print(('es_extended: commande %s ignoré, job pas trouvé !'):format(self.source))
	end
end
