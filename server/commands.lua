...
TriggerEvent('es:addGroupCommand', 'setjob2', 'jobmaster', function(source, args, user)
	if tonumber(args[1]) and args[2] and tonumber(args[3]) then
		...
		if xPlayer then   --problème
			if ESX.DoesJobExist(args[2], args[3]) then
				xPlayer.setJob2(args[2], args[3])
			else
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', "Ce job n'existe pas." } })
			end

		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Joueur pas en ligne.' } })
		end
	else
		TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Mauvaise Utilisation.' } })
	end
end, function(source, args, user)
	TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Permission non suffisante.' } })
end, {help = _U('setjob'), params = {{name = "id", help = _U('id_param')}, {name = "job2", help = _U('setjob2_param2')}, {name = "grade2_id", help = _U('setjob2_param3')}}})
